...
def extract_frames_for_review(video_path, home_team_id, away_team_id, match_day, callback=None):
    ...
    for frame_path in frames:
        ...
        if is_home or is_away:
            ...
            cloudinary_url = None
            try:
                cloudinary_folder = f"frames/{session_id}"
                public_id, url = file_manager.upload_image_to_cloudinary(frame_path, folder=cloudinary_folder)
                if public_id and url:
                    cloudinary_public_id = public_id
                    cloudinary_url = url
                    print(f"Uploaded frame to Cloudinary: {url}")
            except Exception as e:
                print(f"Warning: Failed to upload frame to Cloudinary: {str(e)}")
...
def upload_video():
    ...
    # Upload to Cloudinary in background to avoid blocking
    def upload_to_cloudinary_background():
        try:
            # Upload the file to Cloudinary
            public_id, url = file_manager.upload_file_to_cloudinary(filepath, folder="video_uploads")
            
            with task_lock:
                if upload_id in processing_tasks:
                    processing_tasks[upload_id]['cloudinary_public_id'] = public_id
                    processing_tasks[upload_id]['cloudinary_url'] = url
                    processing_tasks[upload_id]['log_messages'].append({
                        'message': f"Video uploaded to cloud storage",
                        'type': 'info'
                    })
        except Exception as e:
            with task_lock:
                if upload_id in processing_tasks:
                    processing_tasks[upload_id]['log_messages'].append({
                        'message': f"Warning: Cloud upload failed: {str(e)}",
                        'type': 'warning'
                    })
    
    # Start background thread for Cloudinary upload
    cloudinary_thread = threading.Thread(target=upload_to_cloudinary_background)
    cloudinary_thread.daemon = True
...
def review_frames(session_id):
    ...
    for frame in session_data["frames"]:
        ...
        if "cloudinary_url" not in frame or not frame["cloudinary_url"]:
            # Use local path if no Cloudinary URL
            frame["display_url"] = url_for('static', filename=os.path.join('frames', frame["path"]))
        else:
            # Use Cloudinary URL if available
            frame["display_url"] = frame["cloudinary_url"]
    ...
    using_cloudinary = session_data.get("using_cloudinary", False)
