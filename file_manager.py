...
# New functions for Cloudinary integration
...
def upload_file_to_cloudinary(file_path, folder="uploads"):
    ...
    Returns:
        tuple: (public_id, url) of the uploaded file
    """
    response = cloudinary_helper.upload_file(file_path, folder=folder)
    if response:
        return response['public_id'], response['secure_url']
    return None, None

def upload_image_to_cloudinary(image_data, folder="frames"):
    """
    ...
    Returns:
        tuple: (public_id, url) of the uploaded image
    """
    response = cloudinary_helper.upload_image(image_data, folder=folder)
    if response:
        return response['public_id'], response['secure_url']
    return None, None
...
def delete_file_from_cloudinary(public_id):
    ...
    """
    response = cloudinary_helper.delete_file(public_id)
