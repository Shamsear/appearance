...
def extract_frames(video_path, output_dir="frames"):
    ...
    if duration < 30:
        frame_interval = max(1, int(fps / 5))  # Capture 5 frames per second (was 3)
    elif duration < 120:
        frame_interval = max(1, int(fps / 4))  # Capture 4 frames per second (was 2)
    else:
        frame_interval = max(1, int(fps / 3))  # Capture 3 frames per second for longer videos
...
def identify_player_ratings_frames(frames, reader):
    ...
    # Step 1: First identify frames with clear "Player Ratings: Home/Away" headers
    ...
    # Step 2: Process frames that contain player rating values or partial names
    print("Analyzing additional frames for player names...")
    potential_player_frames = []
    
    for frame_path in frames:
        if frame_path in home_frames or frame_path in away_frames:
        ...
        for detection in ocr_results:
            # Look for rating-like values (like 6.5, 7.0)
            if re.match(r'^\d\.\d$', detection[1]):
                contains_player_data = True
                break
                
            # Look for text that might be player names (capitalized words)
            if detection[2] > 0.5 and re.match(r'^[A-Z][a-z]+(\s+[A-Z][a-z]+){0,2}$', detection[1]):
                contains_player_data = True
                break
                
            # Look for common player name text
            common_names = ["ronaldo", "messi", "mbappe", "neymar", "salah", "kane", "benzema", 
                           "lewandowski", "de bruyne", "pogba", "modric", "ramos", "varane", 
                           "alaba", "kroos", "haaland", "suarez", "silva", "muller", "kimmich",
                           "robertson", "alexander-arnold", "trent", "foden", "grealish", "mount"]
            if any(name in detection[1].lower() for name in common_names):
                contains_player_data = True
                break
        
        if contains_player_data:
            potential_player_frames.append(frame_path)
    
    # Process potential player frames
    print(f"Processing {len(potential_player_frames)} additional frames with potential player names")
    for frame_path in potential_player_frames:
        ocr_results = extract_text_from_image(frame_path, reader)
        
        # Try to guess if this is more likely home or away
        likely_home = False
        likely_away = False
        
        # If we can't determine, make a best guess based on filename or other features
        if not likely_home and not likely_away:
            # Look for clues in the text
            all_text = " ".join([detection[1] for detection in ocr_results])
            
            # Default to home if we can't determine otherwise (more common to see home team first)
            likely_home = True
            
        # Process the text - if we think it's a home frame
        if likely_home:
            h_temp, _ = process_text_data(ocr_results)
            # Only add players we don't already have
            for player in h_temp:
                if player and len(player) > 3 and not any(are_similar_names(player, existing) for existing in home_players):
                    print(f"  Found additional home player: {player}")
                    home_players.append(player)
        
        # If we think it's an away frame
        else:
            _, a_temp = process_text_data(ocr_results)
            # Only add players we don't already have
            for player in a_temp:
                if player and len(player) > 3 and not any(are_similar_names(player, existing) for existing in away_players):
                    print(f"  Found additional away player: {player}")
                    away_players.append(player)
    
    # Step 3: Reprocess once more with even more aggressive settings if we still don't have enough players
    if len(home_players) < 11 or len(away_players) < 11:
        print("Didn't find all 11 players, reprocessing ALL frames with aggressive settings...")
        
        for frame_path in frames:
            if frame_path in home_frames or frame_path in away_frames or frame_path in potential_player_frames:
                continue  # Skip already processed frames
                
            ocr_results = extract_text_from_image(frame_path, reader)
            
            # Look for text that might be player names (less strict)
            potential_names = []
            for detection in ocr_results:
                text = detection[1].strip()
                # Skip very low confidence or very short text
                if detection[2] < 0.2 or len(text) < 3:
                    continue
                    
                # Skip if it looks like a UI element or non-name text
                if text.lower() in ["home", "away", "back", "menu", "player ratings", "main menu"]:
                    continue
                ...
                # Consider capitalized words that aren't too long as potential names
                if text[0].isupper() and len(text) <= 20:
                    potential_names.append(text)
            
            # If we found potential names, try to assign them to teams
            if potential_names:
                # For now, we'll just add all remaining names to both teams
                # and let the merging step handle duplicates
                for name in potential_names:
                    # Home players
                    if not any(are_similar_names(name, existing) for existing in home_players):
                        if len(home_players) < len(away_players):  # Prioritize the team with fewer players
                            print(f"  Adding potential home player: {name}")
                            home_players.append(name)
                    
                    # Away players
                    if not any(are_similar_names(name, existing) for existing in away_players):
                        if len(away_players) < len(home_players):  # Prioritize the team with fewer players
                            print(f"  Adding potential away player: {name}")
